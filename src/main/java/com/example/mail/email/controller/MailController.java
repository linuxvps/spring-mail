package com.example.mail.email.controller;

import com.example.mail.email.entity.Mail;
import com.example.mail.email.request.EmailRequest;
import com.example.mail.email.response.ApiResponse;
import com.example.mail.email.service.EmailService;
import com.example.mail.email.services.MailServices;
...
//        emailService.sendEmail(request.to(), request.subject(), request.body());
    ...
    @GetMapping("/all")
    public ResponseEntity<List<Mail>> fetchAllMail() {
        List<Mail> mail1 = services.fetchMailList();
        return ResponseEntity.status(HttpStatus.OK).body(mail1);
    }
}
